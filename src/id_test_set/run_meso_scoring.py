import pandas as pd
import torch
from torchvision import transforms
from PIL import Image
from mesonet4_model import MesoNet4

# --- setup model ---
device = 'cpu'
model = MesoNet4()
model.load_state_dict(torch.load('mesonet4_best.pth', map_location=device))
model.eval()
model.to(device)

meso_transform = transforms.Compose([
    transforms.Resize((256, 256)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])

def meso_inference_fn(model, img_path):
    img = Image.open(img_path).convert('RGB')
    tensor = meso_transform(img).unsqueeze(0).to(device)
    with torch.no_grad():
        return model(tensor).item()

def score_all(model, inference_fn, df):
    scores = []
    for i, (_, row) in enumerate(df.iterrows()):
        s = inference_fn(model, row['image_path'])
        scores.append(s)
        if i % 500 == 0:
            print(f"Scored {i}/{len(df)}")
    df = df.copy()
    df['fake_score'] = scores
    return df

def video_level_scores(df):
    return df.groupby('video_id')['fake_score'].mean().reset_index()

master = pd.read_csv('master_faces.csv')

def extract_video_id(path):
    parts = path.split('/')
    return f"{parts[2]}_{parts[-2]}"

master['video_id'] = master['image_path'].apply(extract_video_id)
test_fakes = master[(master['split'] == 'test') & (master['label'] == 1)]

print(f"Scoring {len(test_fakes)} frames with MesoNet...")

meso_scored = score_all(model, meso_inference_fn, test_fakes)
meso_scored.to_csv('meso_scored_frames.csv', index=False)

meso_video = video_level_scores(meso_scored)
meso_video.to_csv('meso_video_scores.csv', index=False)

print("\nDone. Hardest 10 videos for MesoNet (lowest fake_score = most confusing):")
print(meso_video.sort_values('fake_score').head(10))
